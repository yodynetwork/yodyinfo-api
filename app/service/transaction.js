...
class TransactionService extends Service {
  async getTransaction(id) {
    ...
    const {Address: RawAddress} = this.app.yodyinfo.lib
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.yodyinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.yodyinfo.rpc(this.app.config.yodyinfo.rpc)
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.yodyinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.yodyinfo.lib
  ...
  async transformQRC20Transfers(outputs) {
    const TransferABI = this.app.yodyinfo.lib.Solidity.qrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
  ...
  async transformQRC20UnconfirmedTransfers(outputs) {
    const {OutputScript, Solidity} = this.app.yodyinfo.lib
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.yodyinfo.lib
    const {
